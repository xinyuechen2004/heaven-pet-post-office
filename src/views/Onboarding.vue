...
async function generateCharacterCards() {
  ...
  try {
    ...
    const prompt = `参考图1是已确认的宠物画像和画风，参考图2如有，是原始照片，用来校准真实身份。生成一张 2x2 四宫格宠物角色设定图，四格必须像同一只真实宠物在同一套画风下的不同姿态，不是四只不同动物。无论参考图里是猫、狗、兔子、仓鼠、鸟或其他宠物，都必须锁定原物种、品种感、真实年龄感、胖瘦体型、头身比例、脸型、耳朵/眼睛/鼻口/四肢/尾巴比例、毛发长度、毛色冷暖、明暗关系、花纹/斑点/渐层/特殊标记的位置和形状。禁止改变物种、品种、毛色、花纹、体型、五官比例或性格气质；禁止把宠物画成更萌、更幼、更夸张、更戏剧化或另一只动物。只保留照片里真实存在的配饰，禁止新增铃铛、吊坠、牌子、项圈、蝴蝶结。风格为清晰彩色写实插画，完整全彩上色，线条自然干净，可融入后续明信片场景。四格顺序：左上微侧正坐，右上侧面站着，左下自然小跑，右下侧身趴着。每格只出现宠物本体，完整头到尾，比例一致，纯白背景。禁止白色描边、黄色描边、贴纸边、外轮廓光边、毯子、垫子、地面、阴影底座、装饰物、道具、背景、文字、标签、边框、黑白线稿、素描、铅笔稿、填色书效果、怪物化、异形化、美化、萌化、大眼、圆脸、幼态、Q版、3D、水印。`
